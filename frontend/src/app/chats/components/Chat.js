...
const EmptyState = ({
  ...
  <div className="flex flex-col items-center justify-center min-h-0 h-full overflow-hidden">
    <div className="max-w-md w-full space-y-4 px-4 sm:px-6">
      ...
      >
        <SelectTrigger className="w-full py-6 md:py-8">
...
const MessageList = ({ messages, status }) => (
  <div className="space-y-4">
    {messages.map((msg, index) => (
      <div key={index} className="flex items-start gap-4 micro-card glass-card p-4 border-white/10 transition-all duration-300 hover:shadow-lg animated-border">
        <div
...
export function Chat({
  ...
  return (
    <div className="flex-1 flex flex-col md:max-w-[80%] md:mx-auto w-full h-full">
      <div className="sticky top-0 glass-header z-10 border-b border-white/20 micro-hover">
        <div className="px-8 py-2 pt-8 md:pt-2 flex items-center justify-between gap-4">
          <h1 className="text-base font-semibold truncate gradient-text">{projectTitle}</h1>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="h-8 w-8 micro-button glow-accent transition-all duration-200 hover:shadow-lg"
                      onClick={handleShare}
            ...
            <div className="flex items-center gap-2 micro-hover">
              <div
                className={`w-2 h-2 rounded-full ${statusMap[status].color} ${
                  statusMap[status].animate ? 'animate-pulse pulse-glow' : ''
                }`}
              />
      ...
      <div
        className="flex-1 overflow-y-auto p-2 relative glass-card border-0"
      ...
      <div className="border-t border-white/20 p-2 glass-card">
        <ChatInput
